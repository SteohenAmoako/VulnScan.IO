...
// Gets the ID of a URL analysis from VirusTotal
async function getUrlAnalysisId(url: string): Promise<string> {
    const response: any = await fetchWithApiKey('/urls', {
        method: 'POST',
        body: `url=${encodeURIComponent(url)}`,
    });
    // The ID is in the format: u-{sha256 of url}-{scan timestamp}
    // We need to extract the part after 'u-' and before the last '-'
...
export async function getUrlAnalysis(url: string) {
    ...
    const analysisId = await getUrlAnalysisId(url);
